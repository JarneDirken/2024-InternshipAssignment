            ...
            {active ? 
                <div>
                    ...
                    {item.item.Reparations && item.item.Reparations.length > 0 && (
                        <div className="flex flex-col mt-4 p-4 border-t border-gray-300">
                            <h3 className="text-lg font-semibold">Reparations</h3>
                            {item.item.Reparations.map((reparation) => (
                                <div key={reparation.id} className="mt-2">
                                    <p><strong>Date:</strong> {formatDate(reparation.repairDate)}</p>
                                    <p><strong>Status:</strong> {reparation.returnDate ? `Returned on ${formatDate(reparation.returnDate)}` : 'In repair'}</p>
                                    <p><strong>Notes:</strong> {reparation.notes}</p>
                                </div>
                                ))}
                        </div>
                    )}
